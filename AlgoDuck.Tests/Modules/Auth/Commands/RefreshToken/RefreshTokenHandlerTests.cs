...
public sealed class RefreshTokenHandlerTests
{
    ...
    static Session CreateSession(Guid userId, string refreshToken, string prefix, DateTime utcNow, bool revoked, bool expired)
    {
    ...
    public async Task HandleAsync_WhenRefreshTokenWhitespace_ThrowsFluentValidationException()
    {
        ...
        tokenService.Verify(x => x.RefreshTokensAsync(It.IsAny<Session>(), It.IsAny<CancellationToken>()), Times.Never);
    }
