...
public sealed class DisableTwoFactorHandlerTests
{
    ...
    public async Task HandleAsync_WhenUserIdEmpty_ThrowsPermissionException()
    {
        ...
        var handler = new DisableTwoFactorHandler(userManager.Object, new DisableTwoFactorValidator());

        var ex = await Assert.ThrowsAsync<AuthPermissionException>(() =>
            handler.HandleAsync(Guid.Empty, new DisableTwoFactorDto(), CancellationToken.None));
    ...
    public async Task HandleAsync_WhenUserNotFound_ThrowsPermissionException()
    {
        ...
        var ex = await Assert.ThrowsAsync<AuthPermissionException>(() =>
            handler.HandleAsync(userId, new DisableTwoFactorDto(), CancellationToken.None));
    ...
    public async Task HandleAsync_WhenAlreadyDisabled_DoesNothing()
    {
        ...
        await handler.HandleAsync(userId, new DisableTwoFactorDto(), CancellationToken.None);
