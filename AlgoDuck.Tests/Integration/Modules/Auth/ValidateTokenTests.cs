...
public sealed class ValidateTokenTests
{
    ...
    public ValidateTokenTests(ApiCollectionFixture fx)
    {
        _fx = fx;
    }

    [Fact]
    public async Task ValidateToken_WithJwtFromLoginOrDummy_DoesNot500()
    {
        var suffix = Guid.NewGuid().ToString("N")[..10];
        var email = $"validatetoken_{suffix}@test.local";
        var username = $"validatetoken_{suffix}";
        ...
        var resp = await client.PostAsJsonAsync("/api/auth/validate-token", payload, CancellationToken.None);
