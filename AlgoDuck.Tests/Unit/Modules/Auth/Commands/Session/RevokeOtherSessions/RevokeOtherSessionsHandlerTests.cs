...
namespace AlgoDuck.Tests.Unit.Modules.Auth.Commands.Session.RevokeOtherSessions;
...
public sealed class RevokeOtherSessionsHandlerTests
{
    ...
    public async Task HandleAsync_WhenOnlyCurrentSessionExists_DoesNotSave()
    {
        ...
        repo.Setup(x => x.GetUserSessionsAsync(userId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<Models.Session> { s });
    ...
    public async Task HandleAsync_WhenOtherSessionsNotRevoked_SetsRevokedAtAndSavesOnce()
    {
        ...
        var current = new Models.Session
        {
            SessionId = currentSessionId,
            UserId = userId,
            RefreshTokenHash = "h",
        ...
        var other2AlreadyRevoked = new Models.Session
        {
            SessionId = Guid.NewGuid(),
            UserId = userId,
            RefreshTokenHash = "h2",
        ...
        repo.Setup(x => x.GetUserSessionsAsync(userId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<Models.Session> { current, other1, other2AlreadyRevoked });
    ...
    public async Task HandleAsync_WhenAllOtherSessionsAlreadyRevoked_DoesNotSave()
    {
        ...
        repo.Setup(x => x.GetUserSessionsAsync(userId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<Models.Session> { current, other });
