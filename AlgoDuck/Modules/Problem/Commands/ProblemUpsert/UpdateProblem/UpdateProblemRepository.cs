...
{
    public Task<Result<Guid, ErrorObject<string>>> UpdateProblemAsync(UpsertProblemDto upsertProblemDto, Guid problemId,
        CancellationToken cancellationToken = default);
}

public class UpdateProblemRepository : IUpdateProblemRepository
{
    private readonly IAwsS3Client _s3Client;
    private readonly ApplicationCommandDbContext _dbContext;

    public UpdateProblemRepository(IAwsS3Client s3Client, ApplicationCommandDbContext dbContext)
    {
        _s3Client = s3Client;
        _dbContext = dbContext;
    }

    public async Task<Result<Guid, ErrorObject<string>>> UpdateProblemAsync(UpsertProblemDto upsertProblemDto,
        Guid problemId, CancellationToken cancellationToken = default)
    {
        ...
        _dbContext.TestCases.RemoveRange(toRemove);
