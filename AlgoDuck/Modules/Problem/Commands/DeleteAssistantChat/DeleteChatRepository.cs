    ...
    ) : IDeleteChatRepository
{
    ...
    {
        var chatsDeleted = await dbContext.AssistantChats.Where(e => e.ProblemId == dto.ProblemId && e.UserId == dto.UserId && e.Name == dto.ChatName)
        ...
        if (chatsDeleted == 0) return Result<DeleteChatDtoResult, string>.Err("Chat not found");
        
        return Result<DeleteChatDtoResult, string>.Ok(new  DeleteChatDtoResult
        {
