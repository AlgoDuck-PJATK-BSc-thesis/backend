...
public class CohortService : ICohortService
{
    ...
    public CohortService(ApplicationCommandDbContext commandDb)
    {
        _commandDb = commandDb;
    }

    public async Task<List<CohortDto>> GetAllAsync() =>
        await _commandDb.Cohorts.Select(c => new CohortDto
        {
            CohortId = c.CohortId,
            Name = c.Name
        }).ToListAsync();
    ...
    public async Task<CohortDto?> GetByIdAsync(Guid id) =>
        await _commandDb.Cohorts.Where(c => c.CohortId == id)
    ...
    {
        var user = await _commandDb.ApplicationUsers.FirstOrDefaultAsync(u => u.Id == currentUserId);

        if (user == null)
            throw new NotFoundException("User not found.");

        var alreadyCreated = await _commandDb.Cohorts.AnyAsync(c => c.CreatedByUserId == currentUserId && c.IsActive);
    ...
    {
        var cohort = await _commandDb.Cohorts.FindAsync(id);
    ...
    {
        var cohort = await _commandDb.Cohorts.FirstOrDefaultAsync(c => c.CohortId == id);
        if (cohort is null || !cohort.IsActive) return false;

        cohort.IsActive = false;

        var members = await _commandDb.ApplicationUsers.Where(u => u.CohortId == id).ToListAsync();
    ...
    {
        var user = await _commandDb.ApplicationUsers.FindAsync(userId);
    ...
    {
        return await _commandDb.ApplicationUsers
            .AsNoTracking()
    ...
    public async Task<List<UserProfileDto>> GetUsersAsync(Guid cohortId) =>
        await _commandDb.ApplicationUsers
    ...
    {
        var baseInfo = await _commandDb.Cohorts
            .AsNoTracking()
        ...
        if (!isAdmin)
        {
            var belongs = await _commandDb.ApplicationUsers
                .AsNoTracking()
        ...
        var members = await _commandDb.ApplicationUsers
            .AsNoTracking()
            .Where(u => u.CohortId == cohortId)
            .Select(u => new UserProfileDto
            {
